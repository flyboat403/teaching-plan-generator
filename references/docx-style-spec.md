...
固定行结构（每学时：统一版 17 行；分层版模块三拆双实操时 18 行；序号从 0 计。上列均为**含表头总行数**；JSON `rows` 不含表头，长度 = 总行数 − 1（统一 16 / 分层 17）。以所加载模板为准，下表为统一版示例）：
...
| 2 | 第一模块：破冰筑基 | 分段带（合并） |
| 3–5 | 导入锚定 / 核心授课 / 小结 | 数据 |
| 6 | 第一次缓冲（a—b分钟）：…，不讲新知识 | 合并（数据样式） |
| 7 | 第二模块：重难点攻坚 | 分段带（合并） |
| 8–10 | 衔接与价值锚定 / 重难点精讲 / 深度提问 | 数据 |
| 11 | 第二次缓冲（a—b分钟）：…，不讲新知识 | 合并（数据样式） |
| 12 | 第三模块：落地实操 | 分段带（合并） |
| 13–15 | 知识串联 / 案例实操 / 要求梳理 | 数据 |
| 16 | 终局复盘（a—b分钟）：梳理核心＋布置课后任务＋留白答疑 | 合并（数据样式） |
